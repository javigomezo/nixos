{
  flake.nixosModules.immichServer = {config, ...}: let
    containerName = "immich";
    directories = [
      "${config.my.vars.dockerVolumes}/immich/server/data/upload"
    ];
  in {
    ...
    virtualisation.oci-containers = {
      containers = {
        "${containerName}" = {
          ...
          autoStart = true;
          volumes = [
            "${config.my.vars.dockerVolumes}/immich/server/data/upload:/usr/src/app/upload"
