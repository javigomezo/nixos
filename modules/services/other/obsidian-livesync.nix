{
  flake.nixosModules.obsidianLiveSync = {config, ...}: let
    containerName = "obsidian-livesync";
    directories = [
      "${config.my.vars.dockerVolumes}/${containerName}/data"
    ];
  in {
    sops = {
      secrets = {
    ...
    virtualisation.oci-containers = {
      containers = {
        ${containerName} = {
          ...
          autoStart = true;
          volumes = [
            "${config.my.vars.dockerVolumes}/${containerName}/data/:/opt/couchdb/data/"
            "${config.my.vars.dockerVolumes}/${containerName}/local.ini:/opt/couchdb/etc/local.ini"
